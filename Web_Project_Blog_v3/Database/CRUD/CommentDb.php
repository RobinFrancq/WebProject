...
class CommentDb {
    ...
    public static function insert($comment) {
        return self::getConnection()->executeSQLQuery("INSERT INTO Comments(userID, tekst, rating, postID, aanmaakDatum, title) VALUES ('?','?','?','?','?','?')", array($comment->userID, $comment->text, $comment->rating, $comment->postID, $comment->dateMade, $comment->title));
    }
