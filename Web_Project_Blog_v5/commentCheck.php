<?php
    ...
    // vervolgens wordt geredirect naar index.php
        ...
        $date_for_database = date ("Y-m-d H:i:s", strtotime($values["date"]));  

        $comment = new Comment(0, $values["userId"], $values["text"], $values["rating"], $values["postId"], $date_for_database, $values["title"]);
        //CommentDb::insert($comment); //Deze functie is undefiend?
