...
class CategoryDb {
    ...
    public static function getCategoryByName($name){
        $result = self::getConnection()->executeSQLQuery('SELECT * FROM Categorieen WHERE naam="?"', array($name));
        if ($result->num_rows == 1) {
            $databaseRow = $result->fetch_array();
            return self::convertRowToObject($databaseRow);
        } else {
    ...
    public static function checkIfNameExists($name){
        $result = self::getConnection()->executeSQLQuery('SELECT * FROM Categorieen WHERE naam="?"', array($name));
        if ($result->num_rows == 1) {
            return true;
        } else {
    ...
    public static function insert($category) {
        return self::getConnection()->executeSQLQuery("INSERT INTO Categorieen(naam) VALUES ('?')", array($category->name));
    }
